...
def search():
    ...
    try:
        ...
        # Attempt to load the image, if it fails, ignore it
        try:
...
def Update():
    ...
    reg_number = None  # Initialize with a default value
    for row in sheet.rows:
        if row[0].value == R1:
            name = row[0]
    ...
    # Check if reg_number is still None after the loop
    if reg_number is not None:
        # Proceed with updating the row using reg_number
        ...
        # Handle the case where no matching registration number was found
