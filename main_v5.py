...
def search():
    ...
    try:
        ...
        try:
            img = (Image.open("Fossil Images/" + str(x1) + ".jpg"))
            resized_image = img.resize((190, 190))
            photo2 = ImageTk.PhotoImage(resized_image)
            lbl.config(image=photo2)
            lbl.image = photo2
...
Button(root, text="Upload", width=19, height=2, font="arial 12 bold",
       bg="#8FBC8F", command=showimage).place(x=1000, y=370)
...
Button(root, text="Reset", width=19, height=2, font="arial 12 bold",
       bg="#8FBC8F", command=Clear).place(x=1000, y=530)

Button(root, text="Exit", width=19, height=2, font="arial 12 bold",
       bg="#8FBC8F", command=Exit).place(x=1000, y=610)
